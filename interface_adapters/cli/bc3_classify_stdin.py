...
def _read_stdin_text() -> str:
    """
    Lee stdin de forma robusta en Windows:
    - Soporta UTF-8 con BOM (utf-8-sig)
    - Evita el error: Unexpected UTF-8 BOM
    """
    data = sys.stdin.buffer.read()
    if not data:
        return ""

    # utf-8-sig elimina BOM automáticamente si existe
    try:
        text = data.decode("utf-8-sig")
    except UnicodeDecodeError:
        # fallback (no debería pasar si el json está en UTF-8)
        text = data.decode("utf-8", errors="replace")

    # doble seguridad: por si quedara BOM como carácter Unicode
    return text.lstrip("\ufeff")
...
def main() -> int:
    ...
    logger.info(
        "CLI bc3 classify. prompt_key=%s model=%s bc3_id=%s descompuestos=%s catalogo=%s top_k=%s",
        req.prompt_key,
