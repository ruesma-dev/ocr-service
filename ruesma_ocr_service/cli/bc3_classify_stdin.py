# ruesma_ocr_service/cli/bc3_classify_stdin.py
...
def main() -> int:
    ...
    if dotenv_path:
        logger.info(".env cargado para CLI BC3: %s", dotenv_path)
    ...
    logger.info(
        "CLI bc3 classify. prompt_key=%s model=%s bc3_id=%s descompuestos=%s catalog_yaml=%s llm_batch_size=%s ids=%s",
        req.prompt_key,
