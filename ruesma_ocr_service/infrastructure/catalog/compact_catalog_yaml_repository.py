# ruesma_ocr_service/infrastructure/catalog/compact_catalog_yaml_repository.py
...
class CompactCatalogYamlRepository:
    ...
    def get_bundle(self) -> CompactCatalogBundle:
        ...
        if (
            ...
            and self._cached_mtime == stat.st_mtime
        ):
    ...
    def _load_bundle(self) -> CompactCatalogBundle:
        ...
        for raw_item in items:
            ...
            group_id = (
                str(group_id_raw).strip()
                if group_id_raw is not None
                else str(rule.get("g") or "").strip()
            )
            group_id = group_id or None

            if not type_code:
                raise ValueError(
                    f"No se pudo resolver tipo para el código {code}."
                )
            if family_id not in families:
                raise ValueError(
                    f"La familia {family_id!r} del código {code} no existe en families."
                )
            if group_id and group_id not in groups:
                raise ValueError(
                    f"El grupo {group_id!r} del código {code} no existe en groups."
                )
            if type_code not in types:
                raise ValueError(
                    f"El tipo {type_code!r} del código {code} no existe en types."
                )
